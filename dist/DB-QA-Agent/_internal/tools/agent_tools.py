...
# 配置视觉大模型与文本模型 (需要用到系统环境变量中的 DASHSCOPE_API_KEY)
_DASHSCOPE_API_KEY = os.environ.get("DASHSCOPE_API_KEY", "sk-5498ce6562884bcf81f3073e857c8a1b")

# 必须显式指定为 `qwenvl_oai`，否则 qwen_agent 会因为 model_server 是 http URL，
# 把它错误地降级成普通文本 OAI 模型，导致图片消息链路被破坏。
vl_llm_cfg = {
    'model': 'qwen-vl-plus',
...
# 动态工厂函数，避免由于模型自身带有长期会话记忆导致解析新内容时混入旧内容（也就是所谓的只认得 GTN 图的问题）
def get_vl_parser():
    return Assistant(
        llm=vl_llm_cfg,
...
#     description = '读取本地的 .txt 或 .md 文本文件，先调用AI总结提取后将其核心要素联同原文本存入数据库。'
#     parameters = [{'name': 'file_path', 'type': 'string', 'description': '文本文件的绝对路径', 'required': True}]
#     def call(self, params: str, **kwargs) -> str:
#         try:
#             path = _resolve_pending_path('file_path', json.loads(params).get('file_path'))
#             path = os.path.abspath(path).replace("\\", "/") if path else "" # 规范化保证跨平台及查重有效
#             if not os.path.exists(path): return "失败：找不到文件"
#
#             ui_queue = kwargs.get('ui_queue', None)
#             if ui_queue:
#                 ui_queue.put({'type': 'preview_file', 'data': path})
...
#
#             # 【真实调用大模型萃取长文本骨架】
#             messages = [{'role': 'user', 'content': f"文本太长，请阅读后返回 JSON(包含title, author, summary):\n{content[:3000]}"}]
#             chunks = list(get_text_parser().run(messages=messages))
#             clean_str = chunks[-1][-1]['content'].replace("```json", "").replace("```", "").strip()
#
#             try:
...
#
#             # 确保 summary 包含所有详细内容
#             detailed_summary = parsed_data.get('summary', '')
#             if not detailed_summary:
#                 detailed_summary = content[:500]  # 如果模型没返回，至少用文本开头
#
#             record = {
#                 'data_type': 'text_doc',
#                 'title': parsed_data.get('title', f"文本记录_{os.path.basename(path)}"),
#                 'author': parsed_data.get('author', 'Unknown'),
#                 'summary': detailed_summary,  # 包含完整的详细内容
#                 'raw_text': content, # 底层完整保留原始纯文本，防丢失
#                 'source_path': path
#             }
#             try:
#                 new_id = storage.insert_record(record)
#                 return f"✅ 文本解析入库成功！\n入库ID: {new_id}\n提取信息如下:\n标题: {record['title']}\n作者: {record['author']}\n生成摘要: {record['summary'][:150]}..."
#             except ValueError as ve: return str(ve)
#         except Exception as e: return f"异常: {str(e)}"
@register_tool('ParseAndInsertTextFile')
class ParseAndInsertTextFile(BaseTool):
