# 向量索引服务：将 SQLite 知识记录同步到 ChromaDB 向量库，支持语义检索
import os
import json
...
class VectorIndexService:
    ...
    def add_record(self, record_id: int, record: Dict[str, Any]) -> int:
        """
        将一条 SQLite 知识记录拆分并写入 ChromaDB 向量索引，返回写入的 chunk 数。
        """
        self._lazy_init()
    ...
    def delete_record(self, record_id: int) -> None:
        """
        删除指定 ID 记录在 ChromaDB 中的全部向量 chunk。
        """
        self._lazy_init()
    ...
    def search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """
        根据查询文本在 ChromaDB 中进行语义相似度检索，返回 top_k 条结果。
        """
        self._lazy_init()
